#!/usr/bin/env python3
"""PicoRouter v0.0.3 - Minimal AI Model Router.
...
def create_config_interactive():
    ...
    endpoint = (
        input("Local endpoint [http://localhost:11434]: ").strip()
    ...
    if input("\n🔑 Add API keys now? (y/n) [n]: ").strip().lower() == "y":
        ...
        while True:
            ...
            if not name:
                break
            rate = input(
                f"  Rate limit for {name} (req/min, empty for default): "
            ).strip()
...
def main():
    ...
    serve_parser.add_argument(
        "--host", default="0.0.0.0", help="Host: localhost, all, tailscale, lan, or IP"
    )
    ...
    serve_parser.add_argument(
        ...
        type=int,
        default=60,
        help="Requests per minute (0 to disable)",
    )
    serve_parser.add_argument(
        "--show-ips", "-i", action="store_true", help="Show available network IPs"
    )
    ...
    key_add.add_argument(
        "--rate-limit", "-r", type=int, default=60, help="Requests per minute"
    )
    ...
    key_add.add_argument(
        "--budget", type=float, help="Budget limit in USD (None = unlimited)"
    )
    key_add.add_argument(
        "--budget-period",
    ...
    secrets_parser = subparsers.add_parser(
        "secrets", help="Provider API key management"
    )
    secrets_parser.add_argument(
        "--backend",
        "-b",
        default=None,
        help="Backend: env, dotenv, vaultwarden, encrypted",
    )
    secrets_subparsers = secrets_parser.add_subparsers(dest="secrets_command")

    secrets_list = secrets_subparsers.add_parser(
        "list", help="List configured provider keys"
    )
    ...
    models_parser = subparsers.add_parser(
        "models", help="Model discovery from models.dev"
    )
    ...
    models_search.add_argument(
        "--context", type=int, default=0, help="Min context length"
    )
    models_search.add_argument("--provider", "-p", help="Filter by provider")
    models_search.add_argument(
        "--limit", "-n", type=int, default=20, help="Max results"
    )
    ...
    models_providers = models_subparsers.add_parser(
        "providers", help="List available providers"
    )

    # NEW: models list command
    models_list = models_subparsers.add_parser("list", help="List cached models")
    models_list.add_argument(
        "--zdr", action="store_true", help="Show only ZDR (privacy) models"
    )
    models_list.add_argument(
        "--refresh", action="store_true", help="Force refresh model cache"
    )
    models_list.add_argument("--json", action="store_true", help="Output as JSON")
    ...
    if args.command == "serve":
        ...
        if args.json:
            ...
            print(
                f"  Total: {summary['total']} | 🟢 {summary['healthy']} | 🟡 {summary['degraded']} | 🔴 {summary['down']} | ⚪ {summary['unknown']}"
            )
            print()
            for p in providers:
                print(
                    f"  {p.indicator} {p.name:12} {p.status:8} | latency: {p.latency_ms:>7.0f}ms | uptime: {p.uptime * 100:>5.1f}%"
                )
                if p.last_error:
