...
class EncryptedFileBackend(SecretsBackend):
    ...
    def _encrypt(self, data: str) -> str:
        ...
        encrypted = bytes(a ^ b for a, b in zip(data.encode(), (key * ((len(data) // len(key)) + 1))[:len(data)]))
        return base64.b64encode(encrypted).decode()
    
    def _decrypt(self, data: str) -> str:
        """Decrypt XOR + base64."""
        key = hashlib.sha256(self.password.encode()).digest()
        decrypted = bytes(a ^ b for a, b in zip(base64.b64decode(data), (key * ((len(base64.b64decode(data)) // len(key)) + 1))[:len(base64.b64decode(data))]))
